...
// Handle sorting & filtering & generation of a custom "status" column for the promos list screen
global $pagenow;
if(is_admin() && 'edit.php' == $pagenow && !empty($_GET['post_type']) && 'wordpress_promotion' == $_GET['post_type']) {
    ...
    function fwp_get_post_ids_for_status($status) {
        ...
        if($status == 'expired') {
            ...
            ";
            $conditions = "
                and p.post_status in ('publish', 'template')
